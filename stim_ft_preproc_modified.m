...
con_labels = {'SPEC_ALL', 'FREE_ALL', 'NULL'};    % Labels of conditions corresponding to trigger codes

offset = zeros(1,3);             % OFFSET between trigger and stimulus presentation, e.g. projector delay (milliseconds)
% for stimulus:
...
load('/imaging/hp02/mmn_08/analysis_spm/channels.mat');
...
% % %         end
% % %         
% % %         if exist(sprintf('befffMs%d_FT_blk%d.mat', sname(ss), ses), 'file') && exist(sprintf('s%d_FT_blk%d.mat', sname(ss), (ses+1)), 'file')
% % %             disp(sprintf('subject s%d, block %d done', sname(ss), ses))
% % %         else
...
% % %         
% % %         filename = D.fname;
% % %         add_chans_types(filename, sname(ss), blkout{ses});
% % %         
% % %         %% Baseline correction
% % %         S = [];
% % %         S.D = D.fname;
...
%%
 parfor ss = 1:nr_sbjs   
    ...
    S=[];
    if ss == 15
        S.D = strvcat(efile2, efile3, efile4);
    else    
        S.D = strvcat(efile1, efile2, efile3, efile4);
    end
    S.recode = 'same';
    D = spm_eeg_merge(S);
    ...
    %     S.methods(end).settings.threshold = 2000; %2500ft%meg_thr(ss);
    %
    ...
    nrej = 0;
    %     spm_eeg_load(D.fname)
    %     for i = 1:length(D.trials)
    %         if D.trials(1,i).bad ==1
    %             nrej = nrej+1;
    %         else
    %             continue
    %         end
